...
func TestMD5HasherHash(t *testing.T) {
	h, err := fsutil.NewMD5Hasher(7).Hash(strings.NewReader("test"))
	if err != nil {
		t.Error(err)
	}
	want := "098f6bc"
...
func TestMD5HasherHashLength(t *testing.T) {
	h, err := fsutil.NewMD5Hasher(100).Hash(strings.NewReader("test"))
	if err != nil {
		t.Error(err)
	}
	want := ""
...
func TestMD5HasherHashError(t *testing.T) {
	h, err := fsutil.NewMD5Hasher(100).Hash(faultyReader{})
	if err != errTest {
...
func TestMD5HasherIsHash(t *testing.T) {
	is := fsutil.NewMD5Hasher(9).IsHash("123abcdef")
	if !is {
...
func TestMD5HasherIsHashFalse(t *testing.T) {
	is := fsutil.NewMD5Hasher(9).IsHash("123abcdeg")
	if is {
...
func TestMD5HasherIsHashLength(t *testing.T) {
	is := fsutil.NewMD5Hasher(5).IsHash("123")
	if is {
